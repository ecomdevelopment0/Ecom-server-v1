...
export const fetchAllProducts = async (req, res) => {
  try {
    ...
    let searchQuery = q.search ? { name: { [Op.like]: `%${q.search}%` } } : {};
...
export const addNewProduct = async (req, res) => {
  try {
    ...
    const pbc = await db.productBrandCategories
      .create({
        brandId,
        categoryId,
    ...
    if (productKeys && productKeys.length !== 0) {
      const keysToInsert = productKeys.map((prodk) => {
        return { productKey: prodk.productKey, productId: newProduct.productId };
      });
...
export const updateProductImage = async (req, res) => {
  try {
    ...
    let imageName = fetchedProduct.imageName
...
export const updateProductDetails = async (req, res) => {
  try {
    const { productId } = req.query;
    const { name, description, marketPrice, actualPrice, brandId, categoryId } =
      req.body;
    if (!productId) throw Error("ProductId not provided");
    ...
    if (isChanged) {
      ...
      updateBrandCategory["brand"] = await db.Brand.findOne({
        where: { brandId: updateBrandCategory.brandId },
      });
      updateBrandCategory["category"] = await db.Category.findOne({
        where: { categoryId: updateBrandCategory.categoryId },
      });
    }
...
export const editProductKey = async (req, res) => {
  try {
    ...
    const existingProductKey = await db.ProductKey.findOne({
      where: { productKeyId },
    });

    if (!existingProductKey) throw Error("Product Key does not exist");
    if (existingProductKey.orderId)
      throw Error("Product Key cannot be changed");
...
export const addNewProductKeys = async (req, res) => {
  try {
    ...
    if (!productKeys || productKeys.length == 0)
      throw Error("Product keys not provided");
